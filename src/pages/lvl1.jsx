...
export default function Lvl1() {
  ...
  const [funcText, setFuncText] = useState(''); // Başlangıçta boş
  ...
  const zoomRef = useRef(1.1); // Sabit Zoom

  const gravity = 0.0002; 
  const friction = 0.999; 

  const levelConfig = { bx: -7, by: 4, tx: 7, ty: -3 };
  ...
  const nextLevel = () => {
    localStorage.setItem('unlockedLevel', 2); // 2. bölüm kilidini aç
    navigate('/lvl2'); // 2. bölüme yönlendir
  };
  ...
  useEffect(() => {
    ...
    const getFuncY = (xVal) => {
      if (!funcText) return 0; // Eğer kutu boşsa düz çizgi (y=0) varsay
      try {
        ...
        return 0; // Geçersiz denklem yazılırsa kodun çökmesini önler
      }
    };
    ...
    <div className="h-screen w-screen bg-[#020617] flex overflow-hidden font-mono text-slate-300">
      <aside style={{ width: `${sidebarWidth}px` }} className="bg-[#0f172a] border-r border-blue-500/30 flex flex-col shadow-2xl z-20 relative">
        <div className="p-6 border-b border-blue-500/20 bg-blue-500/5 text-center">
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter opacity-80 mt-1">BÖLÜM 1: BAŞLANGIÇ</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-[#020617] rounded-lg border border-purple-500/30 relative">
              ...
              <p className="text-[9px] text-slate-500 mt-3 italic leading-relaxed">
                * Topu portala ulaştırmak için doğru matematiksel denklemi yaz.<br/>
                * İpucu: Aşağı doğru bir eğim için negatif sayılar kullan (Örn: <strong className="text-purple-300">-0.5x</strong>).
              </p>
            </div>
      ...
      <main className="flex-1 relative bg-[#020617] overflow-hidden" ref={containerRef}>
        {/* KİLİTLİ HARİTA: onMouseDown, onMouseMove, onWheel silindi */}
