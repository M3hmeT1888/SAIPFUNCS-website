...
export default function Lvl3() {
  ...
  const friction = 1; 
  const timeRef = useRef(0); // Lazer animasyonu için zamanlayıcı

  // BÖLÜM 3 TASARIMI: Koordinatlar ve hedef
  const levelConfig = { bx: -10, by: 10, tx: 10, ty: -5 };
  ...
  // 4. BÖLÜME GEÇİŞ MANTIĞI
  const nextLevel = () => {
    localStorage.setItem('unlockedLevel', 4); // 4. bölüm kilidini aç
    navigate('/lvl4'); // 4. bölüme yönlendir
  };
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      timeRef.current += 0.02; // Lazer kapısı animasyon zamanını ilerlet
      ...
      const osc = Math.sin(timeRef.current) * 3; // Lazerin yukarı-aşağı salınımı
      ...
      // HAREKETLİ LAZER KAPISI ÇİZİMİ
      const obsX = cx + (-2) * currentUnitSize;
      const obsY = cy - (osc + 5) * currentUnitSize;
      const obsW = 4 * currentUnitSize;
      const obsH = 10 * currentUnitSize;
      ...
      // FİZİK GÜNCELLEMESİ
      if (isPlaying) for(let i=0; i<4; i++) updatePhysics(canvas, cx, cy, currentUnitSize, osc);
      animationFrameId = window.requestAnimationFrame(render);
    };

    const updatePhysics = (canvas, cx, cy, currentUnitSize, osc) => {
      const b = ballRef.current;
      b.vy -= gravity; b.x += b.vx; b.y += b.vy;
      const lineY = getFuncY(b.x);

      // HAREKETLİ ENGEL ÇARPIŞMASI
      if (b.x > -2 && b.x < 2 && b.y > (osc - 5) && b.y < (osc + 5)) {
        if (autoStart) ballRef.current = { ...checkpointRef.current, vx: 0, vy: 0 }; else resetSim();
        return;
      }
      
      // Çarpışma ve Eğim (Türev) Hesabı
    ...
    <div className="h-screen w-screen bg-[#020617] flex overflow-hidden font-mono text-slate-300">
      <aside style={{ width: `${sidebarWidth}px` }} className="bg-[#0f172a] border-r border-blue-500/30 flex flex-col shadow-2xl z-20 relative">
        <div className="p-6 border-b border-blue-500/20 bg-blue-500/5 text-center">
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter opacity-80 mt-1">BÖLÜM 3: DOĞRU ZAMAN doğru adam</p>
        </div>
        ...
        <div className="flex-1 p-6 space-y-8 overflow-y-auto">
          <div className="space-y-6">
            ...
            {/* HİKAYE & İPUCU KUTUSU */}
            <div className="p-4 bg-rose-500/10 border border-rose-500/30 rounded-lg text-[11px] text-rose-300 leading-relaxed italic">
              "Dikkat! Ortadaki lazer kapısı yukarı-aşağı hareket ediyor. Sadece denklemi çözmek yetmez, topu doğru saniyede ateşlemelisin!"
            </div>

            {/* SADECE YAZMA MODU ARAYÜZÜ */}
        ...
        {winStatus && (
          <div className="absolute inset-0 bg-[#020617]/90 backdrop-blur-md flex flex-col items-center justify-center z-50">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={nextLevel} className="px-8 py-4 bg-purple-500 text-white font-black rounded-xl shadow-xl hover:scale-105 transition-all tracking-widest">SONRAKİ BÖLÜM</button>
            </div>
          </div>
        )}
